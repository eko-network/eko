...
class _PostCardState extends State<PostCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return (!widget.visible || isBlockedByMe() || blocksMe())
        ? const SizedBox.shrink()
        : Padding(
            padding: const EdgeInsets.only(top: 5, bottom: 0, right: 0),
            child: InkWell(
              onTap: () =>
                  (!widget.isPreview && !widget.isPostPage && isLoggedIn())
                      ? context
      ...
      ''',
                        ...
                        CountDisplay(
                          ...
                          onTap: () {
                            if (isLoggedIn()) {
                              context.push('/feed/post/${post.postId}/likes',
                                  extra: post.postId);
                            }
                          },
                        ),
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
      ...
      ''',
                        ...
                        CountDisplay(
                          ...
                          onTap: () {
                            if (isLoggedIn()) {
                              context.push('/feed/post/${post.postId}/dislikes',
                                  extra: post.postId);
                            }
                          },
                        ),
                        ...
                        InkWell(
                          onTap: () {
                            if (isLoggedIn()) {
                              if (!widget.isPreview && !widget.isPostPage) {
                                context
                        ...
                        InkWell(
                          onTap: () {
                            if (isLoggedIn()) {
                              widget.isPreview ? null : sharePressed();
                            }
                          },
                          child: SvgPicture.string(
                            Platform.isIOS
